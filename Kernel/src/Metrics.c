...
static int mostrar_metricas_por_pantalla(){
	...
	void mostrar_estadisticas_de_memorias_simple(void *memoria){
		printf("\n");
			if(((Memoria*)memoria)->fueAsociada){
				log_info(logger_invisible, "-----Memoria %d %s: %s-----", ((Memoria*)memoria)->numero, ((Memoria*)memoria)->ip, ((Memoria*)memoria)->puerto);
				if(((Memoria*)memoria)->Metrics.SC.estaAsociada)
					log_info(logger_invisible, "Asignada al criterio SC | INSERTS: %d | SELECTS: %d", ((Memoria*)memoria)->Metrics.SC.cantidadInsert, ((Memoria*)memoria)->Metrics.SC.cantidadSelect);
				if(((Memoria*)memoria)->Metrics.SHC.estaAsociada)
					log_info(logger_invisible, "Asignada al criterio HSC | INSERTS: %d | SELECTS: %d", ((Memoria*)memoria)->Metrics.SHC.cantidadInsert, ((Memoria*)memoria)->Metrics.SHC.cantidadSelect);
				if(((Memoria*)memoria)->Metrics.EC.estaAsociada)
					log_info(logger_invisible, "Asignada al criterio EC | INSERTS: %d | SELECTS: %d", ((Memoria*)memoria)->Metrics.EC.cantidadInsert, ((Memoria*)memoria)->Metrics.EC.cantidadSelect);
			}else{
				log_info(logger_invisible, "Memoria %d %s:%s no esta asociada a ningun criterio", ((Memoria*)memoria)->numero, ((Memoria*)memoria)->ip, ((Memoria*)memoria)->puerto);
			}
	}

	log_info(logger_invisible, "-----Operaciones totales----- [%d]", metricas.operacionesTotales);
	log_info(logger_invisible, "-----StrongConsistency-----"STD);
	log_info(logger_invisible, "Read latency: %llu", metricas.At.StrongConsistency.readLatency);
	log_info(logger_invisible, "Write latency: %llu", metricas.At.StrongConsistency.writeLatency);
	log_info(logger_invisible, "Reads: %d", metricas.At.StrongConsistency.reads);
	log_info(logger_invisible, "Writes: %d", metricas.At.StrongConsistency.writes);

	log_info(logger_invisible, "-----HashStrongConsistency-----");
	log_info(logger_invisible, "Read latency: %llu", metricas.At.HashStrongConsistency.readLatency);
	log_info(logger_invisible, "Write latency: %llu", metricas.At.HashStrongConsistency.writeLatency);
	log_info(logger_invisible, "Reads: %d", metricas.At.HashStrongConsistency.reads);
	log_info(logger_invisible, "Writes: %d", metricas.At.HashStrongConsistency.writes);

	log_info(logger_invisible, "-----EventualConsistency-----");
	log_info(logger_invisible, "Read latency: %llu", metricas.At.EventualConsistency.readLatency);
	log_info(logger_invisible, "Write latency: %llu", metricas.At.EventualConsistency.writeLatency);
	log_info(logger_invisible, "Reads: %d", metricas.At.EventualConsistency.reads);
	log_info(logger_invisible, "Writes: %d", metricas.At.EventualConsistency.writes);

	if(!list_is_empty(memoriasExistentes)){
		log_info(logger_invisible, "-----Memorias-----");
